...
class PFBlock(nn.Module):
    ...
    def __init__(self, dim, c2, pool_size=3, mlp_ratio=4.,
                 act_layer=nn.SiLU, norm_layer=GroupNorm,
                 drop=0., drop_path=0.,
                 use_layer_scale=True, down_sampling=True, layer_scale_init_value=1e-5):
...
class C3PF(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=1.0):
        ...
        self.pf = nn.Sequential(*(PFBlock(c2, c2) for _ in range(n)))
        self.m = self.pf
